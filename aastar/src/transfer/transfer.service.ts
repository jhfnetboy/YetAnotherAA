...
export class TransferService {
  ...
  private async createUserOperation(
    ...
    // 获取初始化代码
    const initCode = await this.getInitCode(wallet.address, useAAStarValidator, salt);
    ...
    userOp.signature = await this.signUserOp(userOpHash, privateKey, useAAStarValidator, nodeIds);
  ...
  private async getInitCode(
    ...
    if (useAAStarValidator) {
      ...
      const encodedCall = iface.encodeFunctionData("createAccount", [ownerAddress, salt]);
      return ethers.concat([factoryAddress, encodedCall]);
    }
  }
  ...
  private async signUserOp(
    ...
    if (useAAStarValidator) {
      ...
      try {
        ...
        // 2. 调用BLS签名服务进行签名聚合
        const blsResult = await this.blsService.signMessage(userOpHash, nodeIds);
